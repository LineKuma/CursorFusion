...
describe('ConfigManager', () => {
  ...
  describe('load() — auto-discovery', () => {
    it('should auto-discover .cursorfusionrc.json when no path given', async () => {
      ...
      await fs.promises.writeFile(rcPath, JSON.stringify({
        debug: true,
        logLevel: 'debug',
      }));
    ...
    it('should prioritize .cursorfusionrc.json over other defaults', async () => {
      await fs.promises.writeFile(path.join(tmpDir, '.cursorfusionrc.json'), JSON.stringify({ debug: true }));
      await fs.promises.writeFile(path.join(tmpDir, '.cursorfusionrc.js'), JSON.stringify({ debug: false }));
      await fs.promises.writeFile(path.join(tmpDir, 'cursorfusion.config.json'), JSON.stringify({ debug: false }));
  ...
  describe('load() — .js config file', () => {
    it('should load and parse .js config file as JSON', async () => {
      ...
      // 写入 JSON 格式内容（模拟简化处理）
      await fs.promises.writeFile(jsConfigPath, JSON.stringify({
    ...
    it('should merge .js config values into defaults', async () => {
      ...
      expect(cm.config.logLevel).toBe('debug');
      // 未覆盖的保持默认
      expect(cm.config.maxConcurrentTasks).toBe(DEFAULT_CONFIG.maxConcurrentTasks);
      expect(cm.config.timeout).toBe(DEFAULT_CONFIG.timeout);
      expect(cm.config.retryCount).toBe(DEFAULT_CONFIG.retryCount);
    ...
    it('should ignore unknown extensions (not .json or .js)', async () => {
      ...
      await fs.promises.writeFile(unknownExtPath, JSON.stringify({
        debug: true,
